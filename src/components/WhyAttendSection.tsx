...
function WhyCard({ card, index }: { card: (typeof CARDS)[number]; index: number }) {
  return (
    <motion.div
      ...
      whileInView="visible"
      viewport={{ once: true, margin: '-40px' }}
      // Lighter card bg: #2E2C2C instead of #1E1C1C
      className="relative flex h-[280px] flex-col overflow-hidden rounded-2xl bg-[#2E2C2C] p-6 sm:h-[300px] lg:h-[320px]"
    >
      {/* Label — top left */}
      <p className="font-raleway text-[10px] font-bold uppercase tracking-[0.18em] text-white">
        {card.label}
      </p>

      {/* Description — top right, white */}
      <p className="mt-5 self-end max-w-[55%] text-right font-raleway text-[13px] lg:text-[14px] leading-[1.75] text-white">
        {card.description}
      </p>

      {/* Giant number — always bottom-left, cropped */}
      <div
        className="pointer-events-none absolute bottom-[-28px] left-[-8px] select-none font-syne font-extrabold leading-none"
        aria-hidden="true"
        style={{
          fontSize: 'clamp(140px, 18vw, 170px)',
          backgroundImage: 'linear-gradient(160deg, #2D5AE3 0%, #1E44C8 60%, #1233A8 100%)',
          WebkitBackgroundClip: 'text',
          WebkitTextFillColor: 'transparent',
          backgroundClip: 'text',
